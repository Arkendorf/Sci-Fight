...
custom.draw = function()
  ...
  love.graphics.setCanvas(option_canvas)
  love.graphics.clear()
  local offset = option_scroll[1]
  for i, v in ipairs(icons) do
    local x, y = v.x-option_pos.x, v.y-option_pos.y-7
    if current_slot == "skin" then
      custom.icon_background(x, y+offset, 1, loadouts[current_loadout].skin == v.num)
      custom.draw_skin(v.num, x, y+offset) -- skin
    elseif current_slot == "weapon" then
      custom.icon_background(x, y+offset, 2, loadouts[current_loadout].weapon == v.num)
      love.graphics.draw(weapon_icon[v.num], x, y+offset)
    else
      custom.icon_background(x, y+offset, 3, custom.ability_used(v.num))
      love.graphics.draw(ability_icon[v.num], x, y+offset)
    end
  end
  love.graphics.setCanvas(screen.canvas)
  love.graphics.draw(option_canvas, option_pos.x, option_pos.y+7)
end
...
custom.set_current_slot = function(type)
  ...
  gui.add(4, buttons, {}, infoboxes, scroll)
end
...
custom.get_icon_pos = function(type)
  ...
  if type == "skin" then
    for i, v in ipairs(char_img) do
      ...
      icons[#icons+1] = {x = option_pos.x+x*(icon.w+icon.border)+icon.border+4, y = option_pos.y+y*(h+icon.border)+icon.border+5, w = icon.w, h = h, num = i}
      x = x + 1
      if x > 6 then
        x = 0
        y = y + 1
  ...
  elseif type == "weapon" then
    for i, v in ipairs(weapons) do
      ...
      icons[#icons+1] = {x = option_pos.x+x*(w+icon.border)+icon.border+4, y = option_pos.y+y*(icon.h+icon.border)+icon.border+5, w = w, h = icon.h, num = i}
      x = x + 1
      if x > 2 then
        x = 0
        y = y + 1
  ...
  elseif type > 0 then
    for i, v in ipairs(abilities) do
      if (type > 2 and not v.type) or (type < 3 and v.type == weapons[loadouts[current_loadout].weapon].type) then
        icons[#icons+1] = {x = option_pos.x+x*(icon.w+icon.border)+icon.border+4, y = option_pos.y+y*(icon.h+icon.border)+icon.border+5, w = icon.w, h = icon.h, num = i}
        x = x + 1
      end
      if x > 6 then
        x = 0
