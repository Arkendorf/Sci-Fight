...
settings.load = function()
  ...
  for i, v in ipairs(movement_keys) do
    submenu_gui[2][1][i] = {txt = {func = settings.control_string, args = {i}}, x = pos.x+pos.w-13-control_button.w, y = {t = scroll, i = 1, o = pos.y+(control_button.h+control_button.border)*(i-1)+6}, w = control_button.w, h = control_button.h, func = settings.change_control, args = {i}, range = pos}
  end
  for i, v in ipairs(ability_keys) do
    submenu_gui[2][1][i+5] = {txt = {func = settings.control_string, args = {i+5}}, x = pos.x+pos.w-13-control_button.w, y = {t = scroll, i = 1, o = pos.y+(control_button.h+control_button.border)*(i+4)+6}, w = control_button.w, h = control_button.h, func = settings.change_control, args = {i+5}, range = pos}
  end
...
settings.set_submenu = function(num)
  submenu = num
  gui.remove(2)
  local last_button = submenu_gui[num][1][#submenu_gui[num][1]]
